...
| 功能 | 状态 | 说明 |
|------|------|------|
| Rust GPU 窗口 | ✅ | gpui 三栏布局 |
| 浅色主题 | ✅ | 时尚浅色 UI |
| 输入框 + 键盘事件 | ✅ | 打字/退格/回车 |
| 状态机 | ✅ | Input → PageList → Editor → Batch |
| 页面列表（带勾选） | ✅ | 多选/全选 |
| 子流程缩进 | ✅ | depth 0-2 |
| 单页编辑器 | ✅ | prompt 输入 + 保存返回 |
| ✅ Skill 选择 | ✅ | 右侧面板全局选择 |
| ✅ Imagen Prompt 生成 | ✅ | imagen_prompt.lua |
| ✅ 设计文档 | ✅ | 完整架构说明 |
...
| 功能 | 优先级 | 说明 |
|------|--------|------|
| Lua `/split` LLM 调用 | P0 | 真正调用 API 拆分页面 |
| Lua `/generate_page` | P0 | 每页调用 LLM 生成本页设计 |
| Prompt 预览/复制 | P1 | 在编辑器里显示生成的 prompt |
| 批量 prompt 导出 | P1 | 一键复制所有页面的 prompt |
| 图片展示 | P2 | 加载生成的图片到画布 |
| Imagen API 集成 | P2 | 真实调用 Google API |
| Style Reference | P3 | 风格锁定 |
